...
Can you do data analysis with complex numbers? Surprisingly, it turns out, yes you can and Python supports complex numbers natively which makes it easier. The abstraction that makes it all work is a special flavour of Fourier transform called the discrete-time Fourier transform which plays well under convolution operation. You can even represent discrete probability distributions as polynomials, multiply them together and interpet the result as a convolution of the two distributions (after normalizing the coefficients). 
